...
model {
  ...
  for(i in 1:N_obs){
     target +=  bernoulli_logit_lpmf(resp[i] | Q_ast[i,] * theta_trt + Z_matrix[i,]*b0_site) * repeats[i];
  }
...
array[sum(repeats)] int ypred;
for(i in 1:N_obs){

  if(i==1) ypred[1:repeats[i]] = bernoulli_logit_rng(mean(b0_site) + rep_vector(Q_ast[i,] * theta_trt, repeats[i])) ;
  else ypred[(sum(repeats[1:i-1]) +1):sum(repeats[1:i])] = bernoulli_logit_rng(mean(b0_site) + rep_vector(Q_ast[i,] * theta_trt, repeats[i]));
}
