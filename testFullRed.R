...
#               site = expdat$site, trt = as.numeric(expdat$trt), resp = resp)
  ...
  resp_reduced <- resp_dat %>%
    group_by(site,trt,resp)%>%
    summarise(repeats=n())%>%
    data.frame()
  
  resp.reduced <- as.vector(resp_reduced[,3])
  N_obs.reduced <- dim(resp_reduced)[1]
  N_site.reduced <- length(unique(resp_reduced$site))
  N_trt_groups.reduced <- length(unique(resp_reduced$trt))
  data.reduced <- list(N_obs = N_obs.reduced, N_site = N_site.reduced, N_trt_groups = N_trt_groups.reduced, 
                       site = unlist(resp_reduced$site), trt = as.numeric(resp_reduced$trt), resp = resp.reduced, repeats=resp_reduced$repeats)
