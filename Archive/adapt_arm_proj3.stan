 ...
 data {
  ...
  real alpha_prior_mu; //alpha site prior mean
  real alpha_prior_sd; //alpha site prior mean of sd
...
transformed data { 
  ...
  vector[N_trt_groups-1] trt_prior_mu_qr; // trt group priors mu
  vector[N_trt_groups-1] trt_prior_sd_qr; // trt group prior SD
  trt_prior_mu_qr = R_ast * to_vector(trt_prior_mu);
  trt_prior_sd_qr = R_ast * to_vector(trt_prior_sd);
...
model {
  b0~normal(0,1);
  theta_trt[1]~normal(trt_prior_mu_qr[1],trt_prior_sd_qr[1]);
  theta_trt[2]~normal(trt_prior_mu_qr[2],trt_prior_sd_qr[2]);
  theta_trt[3]~normal(trt_prior_mu_qr[3],trt_prior_sd_qr[3]);
