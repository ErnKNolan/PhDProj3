...
testFull <- function(t,expdat,rho,mod,outdir,int_dat,properties_int,int_prior_mu,int_prior_sd,trt_prior_mu,trt_prior_sd,alpha_prior_mu,alpha_prior_sd,...){
  tic()
  prop <- list(...)
  comp <- (1:t)
  o <- list()
  ...
  return <- tryCatch({
    ...
    data <- list(N_obs = N_obs, N_site = N_site, N_trt_groups = N_trt_groups, 
                 ...
                 alpha_prior_mu = alpha_prior_mu, alpha_prior_sd = alpha_prior_sd)
