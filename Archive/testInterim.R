...
testInterim <- function(t,expdat,rho,mod,outdir,int_dat,...){
  ...
  return <- tryCatch({
    ...
    alphas <- res$summary(variables="alpha_site_raw")
    alpha <- c(mean(alphas$mean),mean(alphas$sd))
    #pp_trt1 etc are the predictive prob that the treatment has the largest beta
    results <- list(data.frame(res$summary(variables=c("pred_prob_trt","pp_trt2","pp_trt3","pp_trt4","ov_fut","theta_trt","b0","b0_site")),time=time),resp=list(resp),alphas=list(alpha))
