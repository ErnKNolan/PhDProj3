...
runSimTrialLate <- function(properties = properties, mod = mod, outdir=outdir, j=j,t=t, adaption=adaption,drop_cut=drop_cut,stop_cut=stop_cut,nblock=nblock,draws=draws) {
  
  #The interim clusters are full clusters at 50% for late interim
  properties$kt2 <- ifelse(properties$k[j] == 15,7,ifelse(properties$k[j]==25,13,NA))
  properties$kt3 <- ifelse(properties$k[j] == 15,7,ifelse(properties$k[j]==25,13,NA))
  properties$kt4 <- ifelse(properties$k[j] == 15,7,ifelse(properties$k[j]==25,13,NA))
  ...
  for(i in 1:nint){
    ...
    if(i==1){
      ...
      clusters <- data.frame(interim = properties_int$interim*i, kt2 = sum(mat[1,])+properties$kt2[j], kt3 = sum(mat[2,])+properties$kt3[j], kt4 = sum(mat[3,])+properties$kt4[j])
    }
    t1clusters <- ifelse(i==1 & properties$k[j]==15,7,
                         ifelse(i==1 & properties$k[j]==25,12,
                         ifelse(i==2 & properties$k[j]==15,11,
                         ifelse(i==2 & properties$k[j]==25,19,NA))))
    #making clusters for the interim dataset
    intclusters <- makeClusters(t=4,nid=properties$n_per_k[j],
                                t1=t1clusters,t2=clusters$kt2,
    ...
    mat[,i] <- assignClusterLate(mat=mat, k=ifelse(properties$k[j]==15,4,6), nblock=1, drops=drops, i=i)
  ...
  clusters <- data.frame(k = properties_int$k, kt2 = sum(mat[1,])+properties$kt2[j], kt3 = sum(mat[2,])+properties$kt3[j], kt4 = sum(mat[3,])+properties$kt3[j])
